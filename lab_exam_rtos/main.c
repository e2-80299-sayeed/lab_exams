...
void vSenderTask(void *pvParam)
{
	...
	uint32_t count = sprintf(msg, "Sender Msg:%ld\r\n", uxQueueMessagesWaiting(xQueue));
	portENTER_CRITICAL();
...
void vReceiverTask(void *pvParam)
{
	...
	while((xQueueReceive(xQueue, msg, portMAX_DELAY)) == pdPASS)
	{
		portENTER_CRITICAL();
		HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen(msg)+1, HAL_MAX_DELAY);
		portEXIT_CRITICAL();
	}
	vTaskDelete(NULL);
}
